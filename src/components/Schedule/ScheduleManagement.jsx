            ...
            <header className="schedule-header">
                <div className="header-left">
                    ...
                    <div className="month-control">
                        <button onClick={() => setCurrentDate(subMonths(currentDate, 1))} className="icon-btn"><ChevronLeft size={18} /></button>
                        <h2>{format(currentDate, 'yyyy年 M月', { locale: ja })}</h2>
                        <button onClick={() => setCurrentDate(addMonths(currentDate, 1))} className="icon-btn"><ChevronRight size={18} /></button>
                    </div>
                </div>
                <div className="header-right">
                    <button className={`icon-btn ${compactCompleted ? 'active' : ''}`} 
                            onClick={() => setCompactCompleted(!compactCompleted)}
                            title="完了タスクの詳細表示切り替え">
                        {compactCompleted ? <EyeOff size={18} /> : <Eye size={18} />}
                    </button>
                ...
                <div className="side-panel" onClick={e => e.stopPropagation()}>
                    <div className="side-header">
                        <div className="header-title">
                            ...
                            <h3>{selectedTask ? '業務詳細' : '新規業務登録'}</h3>
                        </div>
                    ...
                    <form onSubmit={handleSave} className="editor-form">
                        <section className="form-section">
                            <label>業務タイトル</label>
                            <input type="text" value={editForm.title} onChange={e => setEditForm({...editForm, title: e.target.value})} required className="main-input" />
                        </section>
                        ...
                        <div className="editor-actions">
                            {selectedTask && (
                                <button type="button" className="delete-btn" onClick={handleDelete}><Trash2 size={18} /> 削除</button>
                            )}
                            <button type="submit" className="save-btn"><Save size={18} /> 保存</button>
                        </div>
                    </form>
                </div>
            </div>
